// src/app/reclamations/_components/ReclamationList.tsx
"use client";
...
export function ReclamationList({ reclamations }: { reclamations: Reclamation[] }) {
  const [filter, setFilter] = useState<string>("Tous");

  const filteredReclamations =
    filter === "Tous"
      ? reclamations
      : reclamations.filter((rec) => rec.objet.toLowerCase().includes(filter.toLowerCase()));
        ...
        {['Tous', 'Salle', 'Matériel', 'Bus', 'Autre'].map((type) => (
          <button
            ...
            className={`px-4 py-2 rounded-full text-sm font-medium shadow-sm transition-all duration-200 flex items-center gap-2
              ${filter === type ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-800 hover:bg-gray-200'}`}
          >
